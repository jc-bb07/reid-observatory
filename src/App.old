...
const TAB_GROUPS = [
  { id: "econ-group",      label: "Economy & Society",  color: "#34d399", homeTab: "econHome",
    tabs: [
      ...
      { id: "islandStory",   label: "IOM Economy Story"    },
...
function HouseholdHomeTab({ C, onNavigate }) {
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto" }}>
      ...
      <div onClick={() => onNavigate("housingApprenticeshipsBrief")} style={{ cursor: "pointer" }}>
        {card(
          "Brief: Housing & Apprenticeships",
          `The Island needs young working-age residents to sustain its workforce, tax base
          and public services as the population ages — but housing costs are the biggest
          reason people who want to stay are questioning whether they still can. A four-part
          framework — supply, access, funding, and market power — with apprenticeships and
          skills built into the delivery mechanism itself, not bolted on afterwards.`,
          "#fb923c"
        )}
      </div>
...
// ── Auto-sizing iframe ────────────────────────────────────────────────────────
// autoResize=false is for pages that manage their own internal scroll (e.g.
// CSS position:sticky / scrollytelling pages that read window.scrollY inside
// the iframe). Those pages need a real scrolling viewport, not a "grow to fit
// all content" one — growing the iframe changes its own internal vh-based
// layout, which re-reports an even bigger height, compounding every cycle.
// With ~50 x 78vh step sections that showed up as a real bug: reported height
// went 17,530px -> 348,800px -> 6.9M px -> 33.5M px (Chromium's size cap)
// inside about 50ms of load, which is what "flashes then disappears" was.
function AutoIframe({ src, title, theme, autoResize = true, height: fixedHeight = "calc(100vh - 120px)" }) {
  const MAX_AUTO_HEIGHT = 20000; // defensive cap even for well-behaved pages
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (e.data && e.data.type === "resize") {
        const h = Math.min(Number(e.data.height) || 0, MAX_AUTO_HEIGHT);
        setHeight(h + "px");
      }
    };
  ...
  return (
    <iframe
      ...
      style={{
        width: "100%",
        height: autoResize ? height : fixedHeight,
        border: "none",
        display: "block",
...
function AppContent() {
  ...
  const tabContent = {
    ...
    islandStory: (
      <AutoIframe src="/island-that-didnt-grow.html" title="The Island That Didn't Grow" theme={themeName} autoResize={false} />
    ),
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginTop: 8, fontSize: 10, color: C.muted, lineHeight: 1.6 }}>
          © 2026 Coalfinch. Analysis and commentary on this site are licensed for reuse with
          attribution to coalfinch.com. Underlying government statistics remain under the{" "}
          <a href="https://www.gov.im/about-this-site/open-government-licence/" target="_blank" rel="noopener"
            style={{ color: C.muted, textDecoration: "underline", textDecorationColor: C.border2 }}>
