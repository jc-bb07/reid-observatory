...
const TAB_GROUPS = [
  ...
  { id: "about-group",   label: "About",               color: "#94a3b8", homeTab: "about",       tabs: ["about"] },
];
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      <div style={{
        ...
        <div style={{ fontSize: 13, color: C.muted2, lineHeight: 1.8 }}>
          Public data, made usable. Fiscal flows, demographic change, health system
          performance, and privacy risk — across the Isle of Man, Jersey, and Guernsey.
          Built because the data exists but is practically inaccessible, and that gap
          has consequences.
        </div>
      </div>

      <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase",
        letterSpacing: "0.08em", marginBottom: 14 }}>Who made this</div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border2}`,
        ...
        <div style={{ fontSize: 13, color: C.muted2, lineHeight: 1.85 }}>
          Manx first, British second, European third. I think the Island
          punches below its weight when it comes to understanding itself — its own
          fiscal position, its demographic trajectory, how its health system compares —
          and that the gap between data that technically exists and data that anyone
          can actually use is a problem worth fixing.
        </div>
        <div style={{ fontSize: 13, color: C.muted2, lineHeight: 1.85, marginTop: 10 }}>
          I also believe, fairly stubbornly, that places like the Isle of Man — small,
          distinctive, culturally specific — add something to the world that larger
          homogenised economies don't. Protecting that takes more than sentiment.
          It takes understanding the numbers.
        </div>
        <div style={{ fontSize: 13, color: C.muted2, lineHeight: 1.85, marginTop: 10 }}>
          This started as a personal project. It's becoming something more professional,
          though it's still largely one person with too many browser tabs open.
          If you've found something wrong, or something missing,{" "}
          <a href="mailto:observatory@coalfinch.com"
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase",
        letterSpacing: "0.08em", marginBottom: 14 }}>What the bloody hell is a coalfinch</div>

      <div style={{ background: C.surface, border: `1px solid ${C.border2}`,
        borderRadius: 10, padding: "18px 20px", marginBottom: 20 }}>
        <div style={{ fontSize: 13, color: C.muted2, lineHeight: 1.85 }}>
          In Minecraft, there are no canaries. This is — if you think about it —
          a strange omission. The canary in the coal mine is one of the oldest
          data instruments in history: a small creature sent ahead into a dangerous
          place to detect invisible risk, so the miners behind it don't have to find
          out the hard way.
        </div>
        <div style={{ fontSize: 13, color: C.muted2, lineHeight: 1.85, marginTop: 10 }}>
          Minecraft has coal mines. It does not have canaries. So if you want
          the early warning system, you have to invent the bird. Enter the coalfinch —
          a made-up creature doing a very real job.
        </div>
        <div style={{ fontSize: 13, color: C.muted2, lineHeight: 1.85, marginTop: 10 }}>
          That felt like the right name for a project about surfacing risks that
          are technically visible in public data but practically invisible to
          the people who should be paying attention to them.
          Go first. Report back. Try not to die in the mine.
        </div>
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase",
        letterSpacing: "0.08em", marginBottom: 14 }}>This observatory</div>

      <div style={{ background: C.surface, border: `1px solid ${C.border2}`,
        borderRadius: 10, padding: "18px 20px", marginBottom: 20 }}>
        <div style={{ fontSize: 13, color: C.muted2, lineHeight: 1.85 }}>
          Three sections, one underlying argument: small islands are complex systems,
          and the data to understand them is out there if you're prepared to go and get it.
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 14 }}>
          {[
            { label: "Economy & Society", color: "#34d399",
              desc: "Demographics, inflation, unemployment, and productivity across the Crown Dependencies. Population aging and the feedback loops between demography and fiscal sustainability." },
            { label: "Government",        color: "#60a5fa",
              desc: "IoM fiscal flows, departmental budgets, and public sector employment. Where government money comes from, where it goes, and how many people it employs." },
            { label: "Health & Care",     color: "#f87171",
              desc: "Manx Care benchmarked against Jersey, Guernsey, and the NHS. Waiting lists, financial performance, off-island dependency, bed capacity modelling." },
            { label: "Privacy & Data",    color: "#a78bfa",
              desc: "Re-identification risk in small populations. How anonymous is anonymous data when you live somewhere with 84,000 people? The original question that started all of this." },
      ...
      <div style={{ background: C.surface2, border: `1px solid ${C.border2}`,
        ...
        Everything here is sourced from public documents — government accounts, census
        publications, statistical bulletins. Where data is estimated, projected, or
        has known quality issues, that's flagged inline.
      </div>

      <div style={{ fontSize: 11, color: C.muted2, lineHeight: 1.7, marginTop: 16 }}>
        <a href="mailto:observatory@coalfinch.com"
          style={{ color: C.muted2, textDecoration: "none" }}>observatory@coalfinch.com</a>
        {" "}·{" "}
        <a href="https://coalfinch.com" target="_blank" rel="noopener"
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        Government finances, departmental budgets, and public sector employment are in the{" "}
        <strong style={{ color: C.text }}>Government</strong> section.
      </div>
...
function AppContent() {
  ...
  const [tab, setTab] = useState("econHome");
  ...
  const groupOf = (id) => TAB_GROUPS.find(g => g.tabs.includes(id));
  ...
  return (
    <div style={{
      ...
      {/* Nav — clickable supertabs + coloured tab strip */}
      <div style={{
        ...
        {TAB_GROUPS.map((group, gi) => {
          const groupActive = group.tabs.includes(tab);
          return (
            <div key={group.id} style={{ display: "flex", flexDirection: "column" }}>

              {/* Supertab label — clickable, jumps to group's home tab */}
              <button
                onClick={() => setTab(group.homeTab)}
                style={{
                  background: "none", border: "none", cursor: "pointer",
                  fontFamily: "'Inter', system-ui, sans-serif",
                  fontSize: 10, fontWeight: 700,
                  textTransform: "uppercase", letterSpacing: "0.09em",
                  color: groupActive ? group.color : C.muted,
                  padding: "0 16px 5px",
                  whiteSpace: "nowrap",
                  transition: "color 0.15s",
                  textAlign: "left",
              ...
              <div style={{ display: "flex", alignItems: "flex-end" }}>
                {gi > 0 && (
                  <div style={{
                    width: 1, background: C.border2,
                    height: 20, flexShrink: 0, marginRight: 4, alignSelf: "center",
                  }} />
                )}
                {group.tabs
                  .map(id => {
